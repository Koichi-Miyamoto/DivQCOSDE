...
def LegExp(f, maxDeg):
    ...
    for l in range(1, maxDeg+1):
        ...
        integ, _ = sp.integrate.quad(integFunc, -1.0, 1.0)
        coefs[l] = (l + 0.5) * integ

    return LegExpResult(coefs)

def LegExpSPDensity(densFunc0, transDensFunc, maxDeg):
    ...
    for l in range(1, maxDeg+1):
        legPoly = sp.special.legendre(l)
        integFunc = lambda x0, x: legPoly(x) * densFunc0(x0) * transDensFunc(x, x0)
        integ, _ = sp.integrate.dblquad(integFunc, -1.0, 1.0, -1.0, 1.0)
        coefs[l] = (l + 0.5) * integ
